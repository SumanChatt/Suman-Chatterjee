...
class ir_import(orm.TransientModel):
    ...
    def _read_csv(self, record, options):
        ...
        """
	browse_file_name = record.file_name
	extension = ''
	if 'xls' in browse_file_name:
		extension='.xls'
	elif 'xlsx' in browse_file_name:
		extension='.xlsx'
	elif 'csv' in browse_file_name:
		extension='.csv'
	if ('.csv' not in extension):
		...
		if (platform.system() == 'Linux'):
        		xls2csv_location = '/opt/openerp/base_import_temp/'
        	elif (platform.system() == 'Windows'):
        		xls2csv_location = 'D:/b_i_t'
        	fil_name = xls2csv_location+str(datetime.datetime.now()).replace(':','')+'_BI'+extension
        	if not os.path.exists(xls2csv_location):
        		os.makedirs(xls2csv_location)
        	fil_name = fil_name.replace(' ','')
        	fl = open(fil_name,'wb')
        	fl.write(record.file)
        	fl.flush()
        	fl.close
		workbook = xlrd.open_workbook(fil_name)
		...
		while curr_row < num_rows:
			...
			while curr_cell < num_cells:
				...
				if cell_type==5:
					cell_value = str(0).encode('utf8')
				elif cell_type!=1:
					cell_value = str(worksheet.cell_value(curr_row, curr_cell))
				else:
					cell_value = worksheet.cell_value(curr_row, curr_cell).encode('utf8')
				file_content = file_content+cell_value+','
